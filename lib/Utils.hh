...
{
    func = Func (injector::GetBranchDestination (address).get ());
}
...
void RegisterHook(const std::string &pattern, int offset, O &originalFunc, F hookedFunc)
{
...
/*******************************************************/
template<typename F>
void RegisterHook(const std::string &pattern, int offset, F hookedFunc)
{
    void *addr = hook::get_pattern (pattern, offset);
    injector::MakeCALL (addr, hookedFunc);
